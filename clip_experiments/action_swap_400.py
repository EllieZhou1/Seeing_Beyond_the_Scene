...
def main():
    ...
    # #per_human_class["human_class"] = {"pred_human":int, "pred_background":int, "total":int}
    # per_human_class = {}


    # #per_human_class["human_class"] = {"pred_human":int, "pred_background":int, "total":int}
    # per_background_class = {}
    ...
    for key, value in tqdm(mapping.items()):
        ...
        human_label = key.split("/")[0]  # Extract the label from the key
        background_label = value[0].split("/")[0]  # Extract the label from the background path
    ...
    with open (f"/n/fs/visualai-scr/temp_LLP/ellie/slowfast_kinetics/clip_experiments/mix_{mix}_actionswap_per_human_class.json", "w") as f:
        json.dump(per_human_class, f, indent=4)

    with open (f"/n/fs/visualai-scr/temp_LLP/ellie/slowfast_kinetics/clip_experiments/mix_{mix}_actionswap_per_background_class.json", "w") as f:
