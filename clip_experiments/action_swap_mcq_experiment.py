...
def evaluate_clip_on_actionswap():
    # Load the CSV file
    ...
    # Background accuracy counters
    background_correct = 0
    background_top5_correct = 0
    background_label_correct = defaultdict(int)
    background_label_total = defaultdict(int)
    ...
    for idx, row in df.iterrows():
        try:
            ...
            # jpg_count = sum(1 for f in os.listdir(row['action_swap_path']) if f.lower().endswith('.jpg'))
            frame_path = os.path.join(row['action_swap_path'], f"{int(jpg_count / 2):06d}.jpg")
            ...
            if clip_choice == background_choice:
                background_correct += 1

            # Background Top-5 accuracy
            if background_choice in top5_indices:
                background_top5_correct += 1

            # Per-label accuracy for background
            background_label = choices[background_choice - 1]  # Get the actual background label text
            background_label_total[background_label] += 1
            if clip_choice == background_choice:
                background_label_correct[background_label] += 1
    ...
    background_top1_accuracy = background_correct / total_samples if total_samples > 0 else 0
    background_top5_accuracy = background_top5_correct / total_samples if total_samples > 0 else 0
    ...
    # Calculate per-label accuracies for background
    background_per_label_accuracies = {}
    for label in background_label_total:
        background_per_label_accuracies[label] = background_label_correct[label] / background_label_total[label] if background_label_total[label] > 0 else 0
    
    # Write results to text file

    actionswap_results_path = "/n/fs/visualai-scr/temp_LLP/ellie/slowfast_kinetics/clip_experiments/actionswap_mcq_results"

    with open(os.path.join(actionswap_results_path, "actionswap_mcq_results.txt"), 'w') as f:
        f.write("CLIP Evaluation Results on Action-Swap MCQ Dataset\n")
        f.write("=" * 50 + "\n\n")
        ...
        f.write("\n")
        f.write("Background Top-1 Accuracy: {:.4f} ({}/{})\n".format(background_top1_accuracy, background_correct, total_samples))
        f.write("Background Top-5 Accuracy: {:.4f} ({}/{})\n\n".format(background_top5_accuracy, background_top5_correct, total_samples))

        f.write("Per-Label Top-1 Accuracies for Background:\n")
        f.write("-" * 30 + "\n")
        for label, accuracy in sorted(background_per_label_accuracies.items()):
            f.write(f"{label}: {accuracy:.4f} ({background_label_correct[label]}/{background_label_total[label]})\n")
