...
def run_dif_seg(model, model_name, num_seg, tokenizer, generation_config, prompt_num):
    ...
    csv_path = os.path.join(base_dir, "dataset/action_swap/action_swap_mcq_all_fields.csv")
    ...
    for idx, row in tqdm(df.iterrows()):
        ...
        if prompt_num == 1:
            ...
            prompt_text = f'Focus only on the person and their motion. Ignore the background, scene, or surroundings. Based on the person’s posture, appearance, and movement, what is the action being performed? A) {choice_1} B){choice_2} C) {choice_3} D) {choice_4} E) {choice_5}'
        ...
        if choice == mapping[human_choice]:
            pred_human += 1
        if choice == mapping[background_choice]:
            pred_background += 1
        ...
        resultdict[f"row_{idx:06d}"] = {
            "full_path":full_path,
            "gt_label":label,
            "background_label":label_background,
            "prompt":prompt_text,
            "response":response,
            "correct_choice":mapping[human_choice],
            "background_choice":mapping[background_choice],
            "llm_choice":response[0]
        }

        del pixel_values, num_patches_list, response, question
        torch.cuda.empty_cache()

    print(f"Human Accuracy = {pred_human/total:.6f}")
    print(f"Background Accuracy = {pred_background/total:.6f}")

    run.log({
        "Human Accuracy":float(pred_human/total),
        "Background Accuracy":float(pred_background/total)
    })

    os.makedirs("/n/fs/visualai-scr/temp_LLP/ellie/slowfast_kinetics/llm_experiments/actionswap", exist_ok=True)
    basedirr = "/n/fs/visualai-scr/temp_LLP/ellie/slowfast_kinetics/llm_experiments/actionswap"


    with open(os.path.join(basedirr, f"llm_responses_for_prompt_{prompt_num}.json"), "w") as f:
...
run = wandb.init(
    ...
    name=f"{modelname}, Action Swap MCQ, Prompt {prompt_num}",
    mode='online',
