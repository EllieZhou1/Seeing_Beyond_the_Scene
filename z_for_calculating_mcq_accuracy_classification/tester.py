...
def test_epoch(model, epoch, loss_fn, dataloader, isHATActionSwap, dataset_name):
    ...
    with torch.no_grad():
        for batch in tqdm(dataloader, desc='test epoch'):
            ...
            for i in range(labels.size(0)): #go through each sample
                label = labels[i].item()
                output = outputs[i] #50 outputs

                if isHATActionSwap:
                    label_background = labels_background[i].item()
                    result_dict[f"row_{row:06d}"] = {
                        "gt_label":int(label),
                        "gt_label_name":kinetics_id_to_classname[label],
                        "gt_background_label":int(label_background),
                        "gt_label_name_background":kinetics_id_to_classname[label_background],
                        "predicted_probs":{
                            f"{kinetics_id_to_classname[j]}":round(float(output[j]), 6)
    ...
    report_path = f"{CONFIG['model_type']}.txt"
    ...
    with open(f"{CONFIG['model_type']}_mix_{dataset_name}.json", "w") as f:
