...
def create_mcq_csv(input_path: str, output_path: str):
    """
    Transform Mimetics CSV into multiple choice question format.
    ...
    for _, row in df.iterrows():
        ...
        # Create a list of available labels (excluding human label)
        available_labels = [label for label in all_labels if label not in [human_label]]
        
        # Randomly select 4 distractor labels
        distractors = random.sample(available_labels, 4)
        
        # Create the list of 5 choices (label_A, label_B, and 3 distractors)
        choices = [human_label] + distractors
        
        # Randomly shuffle the choices so label doesn't always appear in the same position
        random.shuffle(choices)
        
        # Find which choice positions contain human_label
        human_choice_num = choices.index(human_label) + 1  # +1 because choices are 1-indexed
...
if __name__ == "__main__":
    ...
    # File paths
    input_path = "/n/fs/visualai-scr/temp_LLP/ellie/slowfast_kinetics/dataset/mimetics/new_mimetics_all.csv"
    output_path = "/n/fs/visualai-scr/temp_LLP/ellie/slowfast_kinetics/dataset/mimetics/mimetics_mcq.csv"
