#Created by Ellie Zhou on July 19, 2025
...
output_root_seg = "/n/fs/visualai-scr/temp_LLP/ellie/slowfast_kinetics/dataset/action_swap/new_segmented_action_swap"
output_root_binarymask = "/n/fs/visualai-scr/temp_LLP/ellie/slowfast_kinetics/dataset/action_swap/new_binarymask_action_swap"

og_csv_path = "/n/fs/visualai-scr/temp_LLP/ellie/slowfast_kinetics/dataset/action_swap/action_swap_all.csv"

final_csv_path_all = "/n/fs/visualai-scr/temp_LLP/ellie/slowfast_kinetics/dataset/action_swap/new_action_swap_all.csv"
final_csv_path_only_hasHuman = "/n/fs/visualai-scr/temp_LLP/ellie/slowfast_kinetics/dataset/action_swap/new_action_swap_only_hasHuman.csv" #the csv where we will put ONLY the 
#samples where a human was found
final_csv_path_only_noHuman = "/n/fs/visualai-scr/temp_LLP/ellie/slowfast_kinetics/dataset/action_swap/new_action_swap_only_noHuman.csv"
...
wandb.init(
    ...
    name="save_segmented_vids_for_HAT_action_swap",             # Optional: run name
    config={
...
# We will initialize YOLO inside each worker now
...
#   - hasPerson: a boolean
def run_yolo_and_seg(row, predictor, yolo, device):
    ...
    vid_name = os.path.basename(action_swap_path)  # 'sample_000000_deadlifting_to_juggling soccer ball_...'
    sample_idx = int(vid_name.split('_')[1])
    num_files_A = row['num_files_A']

    seg_path = os.path.join(output_root_seg, vid_name)
    binarymask_path = os.path.join(output_root_binarymask, vid_name)

    indices = sample_indices(32, num_files_A)
    hasPerson = False
    ...
    for counter, i in enumerate(indices):
        source = os.path.join(action_swap_path, f"{i:06d}.jpg")
    ...
    img0_np = video_np[0]
    results = yolo.predict(img0_np, device=device) #get bbox for the first frame
    print("finished yolo")
    ...
    for i in range(bboxes.shape[0]):
        ...
        class_name = yolo.names[class_idx]
...
def process_single_row(row, predictor, yolo, device):
    segmented_frames, binarymask_frames, hasPerson, all_bboxes_info, vis_img_path = run_yolo_and_seg(row, predictor, yolo, device)

    action_swap_path = row['action_swap_path']
    vid_name = os.path.basename(action_swap_path)  # 'sample_000000_deadlifting_to_juggling soccer ball_...'
    sample_idx = int(vid_name.split('_')[1])

    video_dir_seg = os.path.join(output_root_seg, vid_name)
    video_dir_binarymask = os.path.join(output_root_binarymask, vid_name)
    ...
    new_row['segmented_path'] = video_dir_seg
    new_row['mask_path'] = video_dir_binarymask
...
def gpu_worker(rows, gpu_id):
    device = f'cuda:{gpu_id}'
    model_cfg = "configs/sam2.1/sam2.1_hiera_l.yaml"
    sam2_checkpoint = "/n/fs/visualai-scr/temp_LLP/ellie/slowfast_kinetics/sam2/checkpoints/sam2.1_hiera_large.pt"
    predictor = build_sam2_video_predictor(model_cfg, sam2_checkpoint, device=device)
    yolo = YOLO("yolov5su.pt")  # Ensure each worker has its own YOLO instance

    results = []
    for idx, row_dict in enumerate(rows):
        print(f"[GPU {gpu_id}] Processing row {idx + 1} of {len(rows)}")
        row = pd.Series(row_dict)
        try:
            results.append(process_single_row(row, predictor, yolo, device))
        except Exception as e:
            print(f"[GPU {gpu_id}] Error processing row: {e}")
    return results
...
if __name__ == "__main__":
    ...
    all_rows = [r for group in results for r in group if r is not None]
